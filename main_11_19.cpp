...
{
    //OPTIONS
    std::cout<<"Rezimo pasirinkimas\n";
    std::cout<<"1 karuna + text\n 2 lentelė\n";
    int mode;
    std::cin>>mode;

    std::string crownPath = "pictures/crown.png";//Karūnos pavadinimas
...
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    while (true)//INFINITE LOOP
    {
        ...
        //XXXXXXXXXXXXXXXXXXXXXXXXXXXXX  Put  XXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        if(mode == 1)
        {
            //Put_Square(frame, 0, 0);
            //Put_Png(frame, png, 100, 0);
            std::vector<int> pos = {-1, -1};
            findRedDotCenter(frame, pos);
            if (pos[0] != -1 && pos[1] != -1)
            {
                cv::Mat pngA = OpenImage(crownPath);
                
                cv::resize(pngA, pngA, cv::Size(), 500.0/(double)pngA.cols, 500.0/(double)pngA.rows);
                Put_Png(frame, pngA, pos[0]-250, pos[1]-250);
                //Put_Square(frame, pos[0]-25, pos[1]-25, 50, 50, -1 ,0, 0, 0, 0.8);
            }
            //Frame / text / x / y / scale / thickness / r / g / b / outline

            //Put_Text(frame, "Tekstas", 20, 80, 5, 20, 0, 255, 0); // žalias

            //PutTextRotated(frame, "XXXX", 300, 200, -25.0, 1.2, 2, cv::Scalar(255,255,255));


            std::array<cv::Point2f,4> quad = {
            cv::Point2f(200, 200), // top-left  (arčiau)
            cv::Point2f(700, 260), // top-right (arčiau)
            cv::Point2f(700, 250), // bot-right (toliau -> siauriau)
            cv::Point2f(200, 260)  // bot-left
            };

            PutTextPerspective(frame, "FDSFNHSDOFSDKOJFHNSODFFDS", quad, 1.0, 2, cv::Scalar(255,255,255));
        }
        else if(mode == 2)
        {
            
            infoTable(frame, "pictures/zalgiris.png", "pictures/lrytas.png", 15, 85);
            frame_i++;
